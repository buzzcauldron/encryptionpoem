...
def encrypt_visual(plaintext: str, password: str, delay=0.001, gui=False) -> str:
    ...
    print("\n**the beast feeds** (shuffled order):")
    ...
    for shuffled_idx, (original_idx, pt_bytes) in enumerate(indexed_lines):
        ...
        print(f"\nEncrypting Line {shuffled_idx:02d} (Original Index {original_idx}):")
        ...
        # Move cursor back up to the line we are encrypting
        sys.stdout.write(f"\033[{num_lines}A") 
        # Move cursor to the start of the line being encrypted
        sys.stdout.write(f"  Line {shuffled_idx:02d}: ")
        ...
        for i in range(line_len):
            ...
            sys.stdout.write(f"  Line {shuffled_idx:02d}: {line_out}\r")
        ...
        sys.stdout.write(f"  Line {shuffled_idx:02d}: {final_line_out}\n")
...
def decrypt_visual(token: str, password: str, delay=0.001, gui=False) -> str:
    print("=== **DECRYPTING** (Restoring Line Order) ===")
    
    try:
        final_blob = base64.urlsafe_b64decode(token)
    except Exception:
        sys.exit("corrupted file: invalid base64.")
    ...
    try:
        ...
        sys.exit("wrong password or corrupted file: invalid data map.")
    ...
    if len(pt_lines) != len(shuffle_map):
        ...
        sys.exit(f"wrong password or corrupted file: line count mismatch (map says {len(shuffle_map)}, data has {len(pt_lines)})")
...
def main():
    ...
    p1.add_argument("--delay",type=float,default=0.1); p1.add_argument("--gui",action="store_true")

    p2 = sub.add_parser("decrypt", help="Decrypt base64 token")
    p2.add_argument("token"); p2.add_argument("password")
    p2.add_argument("--delay",type=float,default=0.1); p2.add_argument("--gui",action="store_true")

    pf1 = sub.add_parser("encrypt-file", help="Encrypt a .txt file")
    pf1.add_argument("file"); pf1.add_argument("password")
    pf1.add_argument("--delay",type=float,default=0.1); pf1.add_argument("--gui",action="store_true")

    pf2 = sub.add_parser("decrypt-file", help="Decrypt a .enc file")
    pf2.add_argument("file"); pf2.add_argument("password")
    pf2.add_argument("--delay",type=float,default=0.1); pf2.add_argument("--gui",action="store_true")
