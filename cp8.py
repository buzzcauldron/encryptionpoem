...
Examples:
  # Encrypt plaintext
  python crypt_poem_3.py encrypt "Roses are red, Violets are blue.\nSugar is sweet, And so are you." mypassword
...
def encrypt_visual(plaintext: str, password: str, delay=0.001, gui=False) -> str:
    ...
    terminal_sleep(delay * 10 * num_lines)
    ...
    for shuffled_idx, (original_idx, pt_bytes) in enumerate(indexed_lines):
        ...
        for i in range(line_len):
            ...
            line_out = line_output_array.decode('latin-1')
        ...
        sys.stdout.write(f"  Line {shuffled_idx:02d}: {line_output_array.decode('latin-1')}\n")
        
        # Print remaining lines (move cursor back down and print rest of the lines)
        for i in range(shuffled_idx + 1, num_lines):
             sys.stdout.write(f"  {current_display_lines[i]}\n")
...
def decrypt_visual(token: str, password: str, delay=0.001, gui=False) -> str:
    ...
    try:
        shuffle_map = list(map(int, map_bytes.decode('utf-8').split(',')))
    except ValueError:
    ...
    pt_lines = pt_blob.decode('utf-8', errors='ignore').split('\n')
    ...
    for i in range(len(pt_visual_bytes)):
        if i < len(current_output):
